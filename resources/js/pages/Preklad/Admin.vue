...
const previews = reactive<Record<string, CaptionEvent | null>>({ cs: null, en: null })

const { connectionState, listenerCount, publish, leave } = usePrekladChannel(sessionId, accessToken)
...
async function start(): Promise<void> {
    ...
    const { data, error } = await supabase
        ...
        .insert({ title: sessionTitle.value, status: 'live', started_at: new Date().toISOString() })
        .select('id')
        .single()
...
function handleLang(lang: CaptionLang, tokens: { text: string; is_final?: boolean }[]): void {
...
const vuPercent = computed(() => Math.min(100, Math.round(vuLevel.value * 280)))
</script>
        ...
        <div v-else class="mx-auto max-w-2xl space-y-6 p-6">
            ...
            <div class="space-y-4 rounded-2xl border border-white/10 bg-white/5 p-5">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <div class="h-2 flex-1 overflow-hidden rounded-full bg-white/10">
                        <div class="h-full rounded-full bg-emerald-400 transition-[width] duration-75" :style="{ width: vuPercent + '%' }" />
                    </div>
                </div>

                <!-- Connection status -->
                <div class="flex flex-wrap gap-2 text-xs">
                    <span class="rounded-full px-2.5 py-1" :class="sonioxState === 'open' ? 'bg-emerald-500/20 text-emerald-300' : 'bg-red-500/20 text-red-300'">
                        Soniox cs→en: {{ sonioxState }}
                    </span>
                    <span class="rounded-full px-2.5 py-1" :class="connectionState === 'open' ? 'bg-emerald-500/20 text-emerald-300' : 'bg-amber-500/20 text-amber-300'">
            ...
            <div v-if="phase === 'live'" class="grid gap-4 sm:grid-cols-2">
                <div class="rounded-2xl border border-white/10 bg-white/5 p-4">
                    <p class="mb-2 text-xs font-semibold text-neutral-400">🇨🇿 Čeština</p>
                    <p class="text-sm leading-relaxed">
                        {{ previews.cs?.final }}
                        <span class="italic text-neutral-500">{{ previews.cs?.partial }}</span>
                    </p>
                </div>
                <div class="rounded-2xl border border-white/10 bg-white/5 p-4">
                    <p class="mb-2 text-xs font-semibold text-neutral-400">🇬🇧 English</p>
                    <p class="text-sm leading-relaxed">
                        {{ previews.en?.final }}
                        <span class="italic text-neutral-500">{{ previews.en?.partial }}</span>
                    </p>
                </div>
            </div>
        </div>
