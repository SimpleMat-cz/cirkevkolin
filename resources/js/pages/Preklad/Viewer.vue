...
import { ENABLED_LANGUAGES, UI_STRINGS, langOption } from '@/lib/preklad/languages'
import type { CaptionEvent, CaptionLang, SessionInfo } from '@/lib/preklad/types'
...
const showPicker = computed(() => phase.value === 'live' && !selectedLang.value)
...
const segments = computed<CaptionEvent[]>(() => {
    ...
    return [...byLang[lang].values()].sort((a, b) => a.seq - b.seq).slice(-MAX_SEGMENTS)
})
...
onCaption((payload) => {
    ...
    if (map.size > MAX_SEGMENTS * 2) {
        const oldest = [...map.keys()].sort((a, b) => a - b).slice(0, map.size - MAX_SEGMENTS)
...
<template>
    ...
    <div class="flex min-h-screen flex-col bg-neutral-950 text-neutral-100">
        ...
        <header class="flex items-center justify-between border-b border-white/10 px-4 py-3">
            <span class="text-sm font-semibold tracking-wide text-neutral-400">Církev Kolín</span>
            <div v-if="phase === 'live' && selectedLang" class="flex items-center gap-2">
                <button
        ...
        <!-- Loading -->
        <div v-if="phase === 'loading'" class="flex flex-1 items-center justify-center p-8">
            <div class="h-10 w-10 animate-spin rounded-full border-2 border-white/20 border-t-white/80" />
        </div>

        <!-- No live session -->
        <div v-else-if="phase === 'no-session'" class="flex flex-1 flex-col items-center justify-center gap-4 p-8 text-center">
            <p class="text-2xl font-semibold">{{ ui.notLive }}</p>
            <p class="max-w-sm text-neutral-400">{{ ui.notLiveHint }}</p>
            <a href="/" class="mt-2 rounded-full bg-white/10 px-5 py-2.5 text-sm font-semibold hover:bg-white/20">
                {{ ui.churchSite }}
            </a>
            <p v-if="!isSupabaseConfigured" class="mt-4 text-xs text-amber-400/80">
                Supabase není nakonfigurován (chybí VITE_SUPABASE_URL / VITE_SUPABASE_ANON_KEY).
            </p>
        </div>

        <!-- Language picker -->
        <div v-else-if="showPicker" class="flex flex-1 flex-col items-center justify-center gap-6 p-6">
            <h1 class="text-center text-xl font-semibold text-neutral-300">{{ UI_STRINGS.en.chooseLanguage }}</h1>
            <div class="grid w-full max-w-md gap-3">
                ...
                >
                    ...
                    <span class="text-lg font-semibold">{{ lang.nativeName }}</span>
                </button>
            </div>
        </div>
        ...
        <div v-else class="relative flex flex-1 flex-col overflow-hidden">
            <div v-if="connectionState === 'reconnecting' || connectionState === 'connecting'"
                class="bg-amber-500/20 py-1.5 text-center text-xs text-amber-200">
                {{ ui.connecting }}
            </div>

            <div ref="scrollEl" class="flex-1 space-y-4 overflow-y-auto px-5 py-6 leading-relaxed" @scroll="onScroll">
                <p v-for="seg in segments" :key="seg.seq" :class="FONT_STEPS[fontStep]">
                    <span>{{ seg.final }}</span>
                    <span v-if="seg.partial" class="italic text-neutral-500"> {{ seg.partial }}</span>
                </p>
                <p v-if="segments.length === 0" class="text-center text-neutral-600">…</p>
            </div>
