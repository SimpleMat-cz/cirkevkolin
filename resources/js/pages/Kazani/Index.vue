...
<template>
    <Head>
        ...
        <meta
            name="description"
            content="Poslechněte si kázání z Apoštolské církve Kolín. Filtrujte podle řečníka, série nebo tématu."
        />
    </Head>
    ...
    <PublicLayout>
            ...
            <div class="mx-auto max-w-6xl px-4 sm:px-6">
                <div class="reveal flex flex-wrap items-center gap-3">
                    <div class="relative min-w-[240px] flex-1 sm:flex-none">
                        <Search
                            class="absolute top-1/2 left-4 h-4 w-4 -translate-y-1/2 text-brand-ink/40"
                        />
                        <input
                            v-model="search"
                            type="search"
                            placeholder="Hledat kázání…"
                            class="w-full rounded-full border border-brand-ink/10 bg-white py-2.5 pr-4 pl-11 text-sm text-brand-ink placeholder:text-brand-ink/40 focus:border-brand-primary focus:ring-2 focus:ring-brand-primary/20 focus:outline-none"
                        />
                    </div>
                    ...
                    <button
                        v-if="
                            search || speaker || selectedSeries || topic || year
                        "
                        class="rounded-full bg-brand-coral px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-brand-coral-dark"
                        @click="resetFilters"
                    >
                        Zrušit filtry
                    </button>
        ...
        <section class="bg-brand-cream py-12 sm:py-20">
            <div class="mx-auto max-w-6xl px-4 sm:px-6">
                <div
                    v-if="sermons.data.length"
                    class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3"
                >
                    <div
                        v-for="(sermon, i) in sermons.data"
                        :key="sermon.id"
                ...
                <div
                    v-else
                    class="rounded-3xl border-2 border-dashed border-brand-ink/10 bg-white py-20 text-center"
                >
                    <p class="font-display text-2xl text-brand-ink/40">
                        Žádná kázání nenalezena.
                    </p>
